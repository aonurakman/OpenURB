"""
This script runs PIMAC_v1 in the OpenURB traffic environment with open (predefined) switching.

PIMAC_v1 keeps MAPPO PPO semantics and adds teacher-student coordination learning:
...
from algorithms.pimac_v1 import PIMACV1AEC
...
# Main script to run the PIMAC_v1 experiment.
if __name__ == "__main__":
    ...
    # PIMAC_v1 learner shared by all AVs (decentralized actor + teacher-critic + distillation).
    pimac_v1 = PIMACV1AEC(
        obs_size,
        action_space_size,
        device=device,
    ...
    #
    # We therefore store one joint transition per day into PIMAC_v1's on-policy buffer.
    ...
    for episode in range(training_eps + dynamic_episodes):
        ...
        for agent_id in env.agent_iter():
            ...
            if termination or truncation:
                ...
                action = pimac_v1.act(obs, agent_index=agent_id_to_index[agent_id])
                idx = agent_id_to_index[agent_id]
                obs_batch[idx] = obs
        ...
        if (episode > training_eps) and (episode % switch_interval == 0):
            ...
            for machine_id in machine_agents_copy:
                if machine_id in env.possible_agents:
                    ...
                    machine_agents_copy[machine_id].model = pimac_v1
    ...
    pbar.close()
    env.plot_results()
    losses_pd = pd.DataFrame([{"id": "pimac_v1", "losses": pimac_v1.loss}])
    losses_pd.to_csv(os.path.join(records_folder, "losses.csv"), index=False)
    with open(os.path.join(records_folder, "pimac_v1_loss_history.json"), "w", encoding="utf-8") as f:
