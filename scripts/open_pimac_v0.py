"""
This script runs PIMAC_v0 in the OpenURB traffic environment with open (predefined) switching.

PIMAC_v0 here follows the canonical CTDE setup:
- shared decentralized recurrent actor for AV decisions,
- centralized critic with a permutation/size-invariant Deep-Sets encoder,
...
# Main script to run the PIMAC_v0 experiment.
if __name__ == "__main__":
    ...
    # PIMAC_v0 learner shared by all AVs (decentralized actor + set-based centralized critic).
    pimac_v0 = PIMACV0AEC(
        obs_size,
        action_space_size,
        device=device,
    ...
    #
    # We therefore store one joint transition per day into PIMAC_v0's on-policy buffer.
