...
from algorithms.pimac_v0 import PIMACV0Parallel
...
def main():
    ...
    pimac_v0 = PIMACV0Parallel(obs_size, action_space_size, device=device, **pimac_v0_kwargs)

    out_dir = make_run_dir("simple_spread_dynamic_v3", "pimac_v0")
    ...
    def run_eval(start_seed: int) -> tuple[float, float]:
        det_backup = pimac_v0.deterministic
        ...
        for idx, n_agents in enumerate(eval_counts):
            ...
            while True:
                ...
                for aid in agent_ids:
                    ...
                    obs_dict[aid] = o
                actions = pimac_v0.act_parallel(obs_dict)
        ...
        pimac_v0.deterministic = det_backup
        pimac_v0.set_train_mode()
    ...
    for ep in range(episodes):
        ...
        if (global_step >= learning_starts) and (len(pimac_v0.memory) >= pimac_v0.batch_size):
            pimac_v0.learn()

        new_losses = pimac_v0.loss[loss_start:]
                ...
                torch.save(
                    {
                        ...
                        "seed": seed,
                        "pimac_v0_kwargs": pimac_v0_kwargs,
        ...
        if (ep + 1) % 200 == 0:
            plot_pimac_v0_curves(
                os.path.join(out_dir, "learning_curves.png"),
                episode_rewards,
                eval_steps,
                eval_means,
                eval_stds,
                pimac_v0.loss_history,
                window=50,
            )
            print(
    ...
    with open(os.path.join(out_dir, "pimac_v0_loss_history.json"), "w", encoding="utf-8") as f:
        json.dump(pimac_v0.loss_history, f, indent=2)
    print(f"Saved results to {out_dir}")

    if os.path.exists(best_ckpt_path):
        ckpt = torch.load(best_ckpt_path, map_location="cpu")
        pimac_v0.actor_net.load_state_dict(ckpt["actor_state_dict"])
        pimac_v0.critic.load_state_dict(ckpt["critic_state_dict"])
