...
from algorithms.pimac_v1 import PIMACV1Parallel
...
def main():
    ...
    pimac_v1 = PIMACV1Parallel(obs_size, action_space_size, device=device, **pimac_v1_kwargs)

    out_dir = make_run_dir("simple_spread_dynamic_v3", "pimac_v1")
    ...
    def run_eval(start_seed: int) -> tuple[float, float]:
        det_backup = pimac_v1.deterministic
        pimac_v1.set_eval_mode()
        ...
        for idx, n_agents in enumerate(eval_counts):
            ...
            while True:
                ...
                for aid in agent_ids:
                    ...
                    obs_dict[aid] = o
                actions = pimac_v1.act_parallel(obs_dict)
        ...
        pimac_v1.deterministic = det_backup
        pimac_v1.set_train_mode()
    ...
    for ep in range(episodes):
        ...
        while True:
            ...
            obs_batch = np.zeros((num_agents, obs_size), dtype=np.float32)
            actions_batch = np.zeros(num_agents, dtype=np.int64)
            active_mask = np.zeros(num_agents, dtype=np.float32)
            ...
            next_obs_batch = np.zeros((num_agents, obs_size), dtype=np.float32)
            next_active_mask = np.zeros(num_agents, dtype=np.float32)
            rewards_batch = np.zeros(num_agents, dtype=np.float32)
            for idx, aid in enumerate(agent_ids):
            ...
            state_before = raw_state_before if raw_state_before is not None else obs_batch.reshape(-1)
            state_after = raw_state_after if raw_state_after is not None else next_obs_batch.reshape(-1)

            pimac_v1.store_transition(
        ...
        if (global_step >= learning_starts) and (len(pimac_v1.memory) >= pimac_v1.batch_size):
            pimac_v1.learn()

        new_losses = pimac_v1.loss[loss_start:]
                ...
                torch.save(
                    {
                        ...
                        "seed": seed,
                        "pimac_v1_kwargs": pimac_v1_kwargs,
                        "actor_state_dict": pimac_v1.actor_net.state_dict(),
                        "critic_state_dict": pimac_v1.critic.state_dict(),
                    },
                    best_ckpt_path,
                )

        if (ep + 1) % 200 == 0:
            plot_pimac_v1_curves(
                os.path.join(out_dir, "learning_curves.png"),
                episode_rewards,
                eval_steps,
                eval_means,
                eval_stds,
                pimac_v1.loss_history,
                window=50,
            )
            print(
    ...
    with open(os.path.join(out_dir, "pimac_v1_loss_history.json"), "w", encoding="utf-8") as f:
        json.dump(pimac_v1.loss_history, f, indent=2)
    print(f"Saved results to {out_dir}")

    if os.path.exists(best_ckpt_path):
        ckpt = torch.load(best_ckpt_path, map_location="cpu")
        pimac_v1.actor_net.load_state_dict(ckpt["actor_state_dict"])
        pimac_v1.critic.load_state_dict(ckpt["critic_state_dict"])
