...
def main():
    ...
    vdn = VDN(obs_size, action_space_size, num_agents=n_agents, device=device, **vdn_kwargs)

    out_dir = make_run_dir("coop_line_world", "vdn")
            ...
            while True:
                ...
                for idx, aid in enumerate(agent_ids):
                    if term[aid] or trunc[aid]:
                        continue
                    actions[aid] = vdn.act(obs[aid], agent_index=idx)
    ...
    for ep in range(episodes):
        ...
        new_losses = vdn.loss[loss_start:]
                ...
                torch.save(
                    {
                        ...
                        "seed": seed,
                        "vdn_kwargs": vdn_kwargs,
                        "agent_state_dict": (
                            vdn.agent_net.state_dict() if vdn.share_parameters else vdn.agent_nets.state_dict()
                        ),
                    },
                    best_ckpt_path,
                )
        ...
        if (ep + 1) % 200 == 0:
            ...
            print(
                ...
                f"loss={episode_losses[-1]:.5f} temp={vdn.temperature:.3f}"
            )
    ...
    if os.path.exists(best_ckpt_path):
        ...
        if vdn.share_parameters:
            vdn.agent_net.load_state_dict(ckpt["agent_state_dict"])
            vdn.target_agent_net.load_state_dict(ckpt["agent_state_dict"])
        else:
            vdn.agent_nets.load_state_dict(ckpt["agent_state_dict"])
            vdn.target_agent_nets.load_state_dict(ckpt["agent_state_dict"])
