...
def test_centralized_critic_accepts_smaller_agent_axes():
    """Critic should pad smaller rosters to its configured team width."""
    critic = CentralizedValueCritic(obs_dim=4, num_agents=5, hidden_sizes=(16, 16))
    obs = torch.randn(2, 3, 3, 4)
...
def test_act_uses_argmax_when_deterministic():
    ...
    with torch.no_grad():
        mappo.actor_net.input_layer.weight.zero_()
        mappo.actor_net.input_layer.bias.zero_()
        for layer in mappo.actor_net.hidden_layers:
            layer.weight.zero_()
            layer.bias.zero_()
        for p in mappo.actor_net.rnn.parameters():
            p.zero_()
        mappo.actor_net.policy_head.weight.zero_()
        mappo.actor_net.policy_head.bias.copy_(torch.tensor([0.0, 1.0, 2.0, 3.0]))

    obs = np.array([0.1, -0.2, 0.3], dtype=np.float32)
    action = mappo.act(obs, agent_index=0)
    assert action == 3
...
def test_gae_matches_manual_recursion():
    ...
    adv, ret = mappo._compute_gae(rewards, values, next_values, dones)
...
def test_store_episode_finalization_contains_ppo_fields():
    ...
    mappo.store_episode(obs, actions, rewards, active)
...
def test_learn_updates_parameters_and_logs_metrics():
    ...
    for _ in range(2):
        ...
        mappo.store_episode(obs_batch, actions_batch, rewards_batch, active_mask)

    before_params = [p.detach().clone() for p in mappo.actor_net.parameters()]
    assert len(mappo.memory) == 2

    mappo.learn()

    after_params = [p.detach() for p in mappo.actor_net.parameters()]
    changed = any(not torch.allclose(p0, p1) for p0, p1 in zip(before_params, after_params))

    assert len(mappo.loss) == 1
    assert changed
    assert len(mappo.memory) == 0
    assert len(mappo.loss_history) >= 1

    last = mappo.loss_history[-1]
    expected_keys = {
        "policy_loss",
        "value_loss",
        "entropy",
...
def test_variable_n_batching_smoke():
    ...
    mappo.store_episode(obs_a, actions_a, rewards_a, active_a)
    mappo.store_episode(obs_b, actions_b, rewards_b, active_b)
...
def test_aec_cycle_storage_smoke():
    ...
    for aid in agent_ids:
        ...
        mappo.aec_record(aid, obs=obs, action=1, reward=1.0, next_obs=obs, done=False)
