...
from algorithms.pimac_v3 import (
    PIMACV3,
    PIMACV3AEC,
    PIMACV3ActorRNN,
    PIMACV3Parallel,
...
def test_actor_uses_film_hyper_projection_and_returns_aux():
    """V3 actor keeps FiLM and exposes hypernetwork diagnostics in aux outputs."""
    torch.manual_seed(0)

    actor = PIMACV3ActorRNN(
        obs_dim=5,
        action_dim=3,
        num_hidden=1,
        widths=(8, 8),
...
def test_act_uses_argmax_when_deterministic():
    ...
    with torch.no_grad():
        pimac_v3.actor_net.input_layer.weight.zero_()
        pimac_v3.actor_net.input_layer.bias.zero_()
        for layer in pimac_v3.actor_net.hidden_layers:
            layer.weight.zero_()
            layer.bias.zero_()
        for p in pimac_v3.actor_net.rnn.parameters():
            p.zero_()
        pimac_v3.actor_net.policy_head.weight.zero_()
        pimac_v3.actor_net.policy_head.bias.copy_(torch.tensor([0.0, 1.0, 2.0, 3.0]))

    obs = np.array([0.1, -0.2, 0.3], dtype=np.float32)
    action = pimac_v3.act(obs, agent_index=0)
    assert action == 3


def test_gae_matches_manual_recursion():
    pimac_v3 = PIMACV3AEC(state_size=2, action_space_size=2, gamma=0.9, gae_lambda=0.8)
    ...
    adv, ret = pimac_v3._compute_gae(rewards, values, next_values, dones)
...
def test_store_episode_finalization_contains_ppo_fields():
    ...
    pimac_v3 = PIMACV3AEC(state_size=4, action_space_size=3, buffer_size=8, batch_size=1)
    ...
    pimac_v3.store_episode(obs, actions, rewards, active)

    assert len(pimac_v3.memory) == 1
    ep = pimac_v3.memory[0]
...
def test_learn_updates_parameters_and_logs_metrics():
    ...
    for _ in range(2):
        ...
        pimac_v3.store_episode(obs_batch, actions_batch, rewards_batch, active_mask)

    before_params = [p.detach().clone() for p in pimac_v3.actor_net.parameters()]
    before_base_policy_weight = pimac_v3.actor_net.policy_head.weight.detach().clone()
    assert len(pimac_v3.memory) == 2

    pimac_v3.learn()

    after_params = [p.detach() for p in pimac_v3.actor_net.parameters()]
    after_base_policy_weight = pimac_v3.actor_net.policy_head.weight.detach()
    ...
    assert len(pimac_v3.loss) == 1
    assert changed
    assert base_head_changed
    assert len(pimac_v3.memory) == 0
    assert len(pimac_v3.loss_history) >= 1

    last = pimac_v3.loss_history[-1]
    expected_keys = {
        "policy_loss",
        "value_loss",
        "entropy",
...
def test_variable_n_batching_smoke_without_fixed_max_critic_dependency():
    ...
    pimac_v3.store_episode(obs_a, actions_a, rewards_a, active_a)
    pimac_v3.store_episode(obs_b, actions_b, rewards_b, active_b)
...
def test_parallel_api_smoke():
    ...
    pimac_v3 = PIMACV3Parallel(state_size=3, action_space_size=4)
    obs_dict = {f"agent_{i}": np.random.randn(3).astype(np.float32) for i in range(5)}
    action_dict = pimac_v3.act_parallel(obs_dict)
...
def test_aec_cycle_storage_smoke():
    ...
    pimac_v3 = PIMACV3AEC(state_size=3, action_space_size=4, buffer_size=8, batch_size=1)
    agent_ids = ["a", "b", "c"]
    pimac_v3.aec_begin_cycle(agent_ids)

    for aid in agent_ids:
        obs = np.random.randn(3).astype(np.float32)
        pimac_v3.aec_record(aid, obs=obs, action=1, reward=1.0, next_obs=obs, done=False)

    pimac_v3.aec_end_cycle(done_all=True)
    assert len(pimac_v3.memory) == 1


def test_pimac_v3_alias_points_to_aec():
    pimac_v3 = PIMACV3(state_size=2, action_space_size=2)
    assert isinstance(pimac_v3, PIMACV3AEC)
...
def test_ema_teacher_updates_toward_online_teacher():
    ...
    target_params_before = [parameter.detach().clone() for parameter in pimac_v3.target_teacher.parameters()]

    with torch.no_grad():
        for parameter in pimac_v3.critic.teacher.parameters():
            parameter.add_(0.5)

    pimac_v3._update_target_teacher()

    target_params_after = [parameter.detach().clone() for parameter in pimac_v3.target_teacher.parameters()]
    online_params_after = [parameter.detach().clone() for parameter in pimac_v3.critic.teacher.parameters()]
...
def test_parallel_done_dict_without_all_key_finalizes_only_when_all_true():
    ...
    pimac_v3 = PIMACV3Parallel(state_size=3, action_space_size=2, buffer_size=8, batch_size=1)
    obs = {
    ...
    pimac_v3.store_parallel_step(obs, actions, rewards, next_obs, done_dict={"a": False, "b": True})
    assert len(pimac_v3.memory) == 0

    pimac_v3.store_parallel_step(obs, actions, rewards, next_obs, done_dict={"a": True, "b": True})
    assert len(pimac_v3.memory) == 1

    pimac_v3_2 = PIMACV3Parallel(state_size=3, action_space_size=2, buffer_size=8, batch_size=1)
    pimac_v3_2.store_parallel_step(obs, actions, rewards, next_obs, done_dict={"__all__": True, "a": False, "b": False})
    assert len(pimac_v3_2.memory) == 1
...
def test_parallel_terminal_empty_next_obs_dict_finalizes_without_crash():
    ...
    pimac_v3.store_parallel_step(obs, actions, rewards, next_obs_dict={}, done_dict={"__all__": True})
...
def test_parallel_act_accepts_non_integer_agent_ids():
    ...
    pimac_v3 = PIMACV3Parallel(state_size=4, action_space_size=3)
    obs_step1 = {
        "agent_alpha": np.random.randn(4).astype(np.float32),
        "agent_beta": np.random.randn(4).astype(np.float32),
    }
    actions_step1 = pimac_v3.act_parallel(obs_step1)
    ...
    actions_step2 = pimac_v3.act_parallel(obs_step2)
    assert set(actions_step2.keys()) == set(obs_step2.keys())

    action_single = pimac_v3.act(np.random.randn(4).astype(np.float32), agent_index=("tuple", 1))
