...
def test_act_switches_between_stochastic_and_deterministic_modes():
    ...
    with torch.no_grad():
        ...
        for layer in layers:
            layer.weight.zero_()
            layer.bias.zero_()
        ppo.policy_net.out_layer.weight.zero_()
        ppo.policy_net.out_layer.bias[:] = torch.tensor([0.3, -0.1])
...
def test_learn_updates_policy_parameters_and_clears_memory():
    ...
    with torch.no_grad():
        ...
        for layer in layers:
            ...
            layer.bias.zero_()
        ppo.policy_net.out_layer.weight.zero_()
        ppo.policy_net.out_layer.bias[:] = torch.tensor([0.2, -0.2])
