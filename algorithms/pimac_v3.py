"""
PIMACV3 implementation for OpenURB.
...
class PIMACV3Base(BaseLearningModel):
    """
    Shared PIMACV3 core used by both AEC and parallel wrappers.
...
class PIMACV3AEC(PIMACV3Base):
    """PIMACV3 wrapper for AEC/OpenURB-style environments."""
...
class PIMACV3Parallel(PIMACV3Base):
    """PIMACV3 wrapper for parallel multi-agent environments."""
    ...
    def act(
        ...
        if agent_index is None:
            raise ValueError("PIMACV3Parallel.act requires agent_index. Prefer act_parallel for parallel environments.")
        return self._act_single(state=state, actor_key=agent_index)
...
# Compatibility alias for call sites that expect `PIMACV3` directly.
PIMACV3 = PIMACV3AEC
