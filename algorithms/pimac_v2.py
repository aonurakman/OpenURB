"""
PIMACV2 implementation for OpenURB.
...
class PIMACV2Base(BaseLearningModel):
    """
    Shared PIMACV2 core used by both AEC and parallel wrappers.
    ...
    def __init__(
        ...
        # Compatibility metadata only; critic width does not depend on this.
    ...
    def _compute_update_diagnostics(
        ...
        """Compute training diagnostics logged to `last_losses` and `loss_history`."""
        with torch.no_grad():
    ...
    def set_eval_mode(self) -> None:
        """Switch actor/critic modules to evaluation mode; target teacher always stays eval."""
        self.actor_net.eval()
        self.critic.eval()
        self.target_teacher.eval()
...
class PIMACV2AEC(PIMACV2Base):
    """PIMACV2 wrapper for AEC/OpenURB-style environments."""
...
class PIMACV2Parallel(PIMACV2Base):
    """PIMACV2 wrapper for parallel multi-agent environments."""
    ...
    def act(
        ...
        if agent_index is None:
            raise ValueError("PIMACV2Parallel.act requires agent_index. Prefer act_parallel for parallel environments.")
        return self._act_single(state=state, actor_key=agent_index)
...
# Compatibility alias for call sites that expect `PIMACV2` directly.
PIMACV2 = PIMACV2AEC
